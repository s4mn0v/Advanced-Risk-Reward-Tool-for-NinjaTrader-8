...
namespace NinjaTrader.NinjaScript.DrawingTools
{
    public class Sfourm : DrawingTool
    {
        ...
        private double GetRealEntryPrice(Position openPosition)
        {
            return AttachedTo.Instrument.MasterInstrument.RoundToTickSize(openPosition.AveragePrice);
        }

        private double GetRealDollarAmount(double levelPrice, double realEntryPrice, double pointValue, Position openPosition)
        {
            double distance = Math.Abs(levelPrice - realEntryPrice);
            double realQty = Math.Abs(openPosition.Quantity);

            double dollarPerContract = AttachedTo.Instrument.MasterInstrument.InstrumentType == InstrumentType.Forex
                ? distance * pointValue * openPosition.Account.ForexLotSize
                : distance * pointValue;

            return Math.Round(dollarPerContract * realQty, 2);
        }
        ...
        private double GetCurrentMarketPrice()
        {
            try
            {
                if (AttachedTo != null && AttachedTo.Instrument != null &&
                    AttachedTo.Instrument.MarketData != null && AttachedTo.Instrument.MarketData.Last != null)
                    return AttachedTo.Instrument.MarketData.Last.Price;
            }
            catch { }

            try
            {
                ChartBars chartBarsForPrice = GetAttachedToChartBars();
                if (chartBarsForPrice != null && chartBarsForPrice.Bars != null && chartBarsForPrice.Bars.Count > 0)
                    return chartBarsForPrice.Bars.GetClose(chartBarsForPrice.Bars.Count - 1);
            }
            catch { }
        ...
        private string GetPriceString(double price, ChartBars chartBars)
        {
            ...
            {
                double realEntry = GetRealEntryPrice(openPosition);
                double realQty = Math.Abs(openPosition.Quantity);
                double realDollar = GetRealDollarAmount(price, realEntry, pointValue, openPosition);
            ...
            if (pct == 0)
            {
                ...
                {
                    double realEntry = GetRealEntryPrice(openPosition);
                    double currentPrice = GetCurrentMarketPrice();
                    double unrealized = 0;

                    if (currentPrice.ApproxCompare(0) != 0)
                    {
                        double dollars = GetRealDollarAmount(currentPrice, realEntry, pointValue, openPosition);
                        bool isProfit = openPosition.MarketPosition == MarketPosition.Long
                            ? currentPrice > realEntry
                            : currentPrice < realEntry;
                        unrealized = isProfit ? dollars : -dollars;
                    }

                    str = string.Format("ENTRY real: {0}  (Qty: {1} | P&L actual: {2})",
                        chartBars.Bars.Instrument.MasterInstrument.FormatPrice(realEntry),
        ...
        private string GetPriceStringPartials(double price, ChartBars chartBars, int numero)
        {
            ...
            if (openPosition != null)
            {
                double realEntry = GetRealEntryPrice(openPosition);
                double realQty = Math.Abs(openPosition.Quantity);
                double realDenom = (price - realEntry) * pointValue;

                double lotMultiplier = AttachedTo.Instrument.MasterInstrument.InstrumentType == InstrumentType.Forex
                    ? openPosition.Account.ForexLotSize
                    : 1;

                double perContractAtLevel = ratio.ApproxCompare(0) == 0
                    ? 0
                    : Math.Abs((realDenom / ratio) * numero) * lotMultiplier;

                double realDollar = Math.Round(perContractAtLevel * realQty, 2);
